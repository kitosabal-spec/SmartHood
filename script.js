...
function seedData() {
  if (!localStorage.getItem('sah_seeded')) {
    ...
    const complaints = [
      ...
      { id: 'c002', homeownerId: 'u003', category: 'Maintenance', description: 'The street light in front of Block 1, Lot 2–4 has been out for over a week. The area is very dark at night and feels unsafe for walking.', status: 'Open', adminResponse: '', dateFiled: '2025-04-18', updatedAt: null, resolvedAt: null },
...
function buildSidebar() {
  ...
  const openComplaints = db.get('complaints').filter(c =>
    currentUser.role === 'admin'
      ? c.status === 'Open'
      : c.homeownerId === currentUser.id && c.status === 'Open'
  ).length;
...
function openManageComplaint(id) {
  ...
  const statusOptions = ['Open', 'In Progress', 'Resolved'].map(s =>
  ...
  openModal(`Manage Complaint — ${ho ? ho.name : 'Unknown'}`, `
    ...
    <div class="form-group">
      <label>Admin Response <span style="color:var(--text-3);font-weight:400">(required to resolve)</span></label>
...
function saveComplaintManagement(id) {
  ...
  if (newStatus === 'Resolved' && !newResponse) {
    showToast('error', 'Response Required', 'Please provide a response before resolving the complaint.');
    return;
  }
...
// SECTION 11: ADMIN — ANNOUNCEMENTS
    ...
    <div class="announcement-card" style="--card-accent:${catColors[a.category]||'#2271c3'}">
      <div class="announcement-header">
        ...
        <div style="display:flex;gap:6px">
          <button class="btn btn-danger btn-sm btn-icon" onclick="confirmDeleteAnnouncement('${a.id}')" title="Delete Announcement"><svg width="14" height="14"><use href="#ico-trash"/></svg></button>
        </div>
      </div>
...
function renderReports() {
  ...
  <div class="charts-row" style="margin-bottom:22px">
    ...
    <div class="chart-card">
      ...
      <div class="donut-legend">
        <div class="legend-item"><div class="legend-dot" style="background:#dc2626"></div> Open (${complaints.filter(c=>c.status==='Open').length})</div>
  ...
  renderDonut('complaintDonut', [
    { value: complaints.filter(c=>c.status==='Open').length,        color: '#dc2626' },
...
function renderHODashboard() {
  ...
  const openComplaints = myComplaints.filter(c => c.status === 'Open' || c.status === 'In Progress');
...
function submitHOComplaint() {
  ...
  addNotification('Complaint Submitted', `Your ${category} complaint has been filed and is now Open.`);
